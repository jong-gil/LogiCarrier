...
echo "build"
sudo docker run -d --name "$service-2" -p 8000:8000 --network logicarrier-network -e "eureka.client.serviceUrl.defaultZone=http://discovery-service:8761/eureka/" $service:$version
